...
import {PostAllParams} from './model/PostAllParams';
...
export class PostController {
    ...
    @Get("/get/all")
    findPosts(
        @Query() params: PostAllParams
    ): Post[] {
        const databasePosts = database.findPosts({
            onlySpotlight: params.onlySpotlight === "true",
            sortDescendingByKey: params.sortDescendingByKey,
            limit: params.limit
        });
